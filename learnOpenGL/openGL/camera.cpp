...
public:
    ...
    void update()
    {
        ...
        // process logic
        if (was_pressed(GLFW_KEY_W))
            speed_x = speed_x < MaxSpeed ? speed_x + speed_delta : MaxSpeed;
        else if (was_pressed(GLFW_KEY_S))
            speed_x = speed_x > -MaxSpeed ? speed_x - speed_delta : -MaxSpeed;
        else
            speed_x = fabs(speed_x) < 0.01f ? speed_x = 0 : speed_x - sgn(speed_x) * speed_delta;

        if (was_pressed(GLFW_KEY_UP))
            Pitch += 2 * delta_time;
        if (was_pressed(GLFW_KEY_DOWN))
            Pitch -= 2 * delta_time;
        if (was_pressed(GLFW_KEY_LEFT))
            Yaw -= 2 * delta_time;
        if (was_pressed(GLFW_KEY_RIGHT))
            Yaw += 2 * delta_time;
